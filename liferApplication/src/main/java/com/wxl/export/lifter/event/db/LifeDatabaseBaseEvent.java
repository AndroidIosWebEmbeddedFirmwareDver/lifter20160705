...
public abstract class LifeDatabaseBaseEvent extends MobileCashBaseEvent implements MobileCashBaseDBTableCreateableInterface
{
	protected List<Cursor> cursors;// 游标集合
	protected boolean isRead = true;// 是否读取

	public LifeDatabaseBaseEvent(int nEventCode)
	{
		super(nEventCode);
	}
	...
	{
		isRead = bRead;
		DatabaseManager dm = LiferDatebaseManager.getInstance();
		if (bRead)
		{
