 ...
 * Android 4.2+ source & the {@link AbsListView} &
 ...
 * <p/>
 * Note: we only really extend {@link AbsListView} so we can
 ...
 * some {@link AdapterView} code is repeated here Be careful with
...
public abstract class ExtendableListView extends AbsListView
{
		...
		try
		{
			...
			} else if (mItemCount != mAdapter.getCount())
			{
				throw new IllegalStateException("The content of the adapter has changed but " + "ExtendableListView did not receive a notification. Make sure the content of "
						+ "your adapter is not modified from a background thread, but only " + "from the UI thread. [in ExtendableListView(" + getId() + ", " + getClass() + ") with Adapter("
						+ mAdapter.getClass() + ")]");
			}
	 ...
	 * {@link ViewGroup.LayoutParams} since they're package private
	...
	public static class ListSavedState extends ClassLoaderSavedState
	{
		 ...
		 * {@link AbsListView#onSaveInstanceState()}
		 */
